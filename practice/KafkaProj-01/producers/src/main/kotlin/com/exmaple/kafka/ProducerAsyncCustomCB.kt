...
//    props.setProperty("bootstrap.servers", "localhost:9092");
    ...
    val kafkaProducer: KafkaProducer<Int, String> = KafkaProducer<Int, String>(props)

    //key가 없기 때문에 partitioner 전략에 따름(2.4 이후부터는 sticky)
    //결과 출력하면, 하나의 partition 의 offset 5까지 쌓인 후 다른 partition 에 쌓이는 것을 볼 수 있음.
    for (i in 0..19) {
        val producerRecord: ProducerRecord<Int, String> = ProducerRecord(topic, i ,"hi $i")
        val callback  = CustomCallback(i);
